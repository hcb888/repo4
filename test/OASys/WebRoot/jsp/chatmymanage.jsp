...
<html>
  <head>
    ...
    <title>My JSP 'chatmymanage.jsp' starting page</title>
...
.paixu:HOVER {
  ...
  <body>
	...
	<div class="col-md-2">
		<h1 style="font-size: 24px; margin: 0;" class="">我的管理</h1>
	</div>
	<div class="col-md-10 text-right">
		<a href="index"><span class="glyphicon glyphicon-home"></span> 首页</a>
		> <a disabled="disabled">我的管理</a>
	</div>
</div>
...
<div class="bgc-w box box-primary">
	...
	<div class="box-header">
		<h3 class="box-title">
					<a href="chatadminadd.html" class="label label-success" style="padding: 5px;">
		...
		<div class="table-responsive">
			<table class="table table-hover">
				...
				<tr>
					...
					<td><span>附件</span></td>
					<td>
						<a href="chatedit.html?id=44" class="label xiugai"><span
							class="glyphicon glyphicon-edit"></span> 修改</a> 
						<a href="replaymanage.html?id=44&pageNumber=0" class="label xiugai"> <span
							class="glyphicon glyphicon-search"></span>查看</a>
						 <a href="javascript:void(0);" discussId="44"
				...
				<tr>
					...
					<td><span>附件</span></td>
					<td>
						<a href="chatedit.html?id=13" class="label xiugai"><span
							class="glyphicon glyphicon-edit"></span> 修改</a> 
						<a href="replaymanage.html?id=13&pageNumber=0" class="label xiugai"> <span
							class="glyphicon glyphicon-search"></span>查看</a>
						 <a href="javascript:void(0);" discussId="13"
						class="label shanchu deletethis"><span class="glyphicon glyphicon-remove"></span>
							删除</a>
					</td>
				</tr>
				<tr>
					...
					<td><span>附件</span></td>
					<td>
						<a href="chatedit.html?id=45" class="label xiugai"><span
							class="glyphicon glyphicon-edit"></span> 修改</a> 
						<a href="replaymanage.html?id=45&pageNumber=0" class="label xiugai"> <span
							class="glyphicon glyphicon-search"></span>查看</a>
						 <a href="javascript:void(0);" discussId="45"
						class="label shanchu deletethis"><span class="glyphicon glyphicon-remove"></span>
							删除</a>
					</td>
				</tr>
				<tr>
					...
					<td><span>附件</span></td>
					<td>
						<a href="chatedit2.html?id=46" class="label xiugai"><span
							class="glyphicon glyphicon-edit"></span> 修改</a> 
						<a href="replaymanage2.html?id=46&pageNumber=0" class="label xiugai"> <span
							class="glyphicon glyphicon-search"></span>查看</a>
						 <a href="javascript:void(0);" discussId="46"
						class="label shanchu deletethis"><span class="glyphicon glyphicon-remove"></span>
							删除</a>
					</td>
				</tr>
...
<script>
		...
		$('.tablefirst').on('click',function(){
			 if(true==false){
				 $('.thistable').load('/metable?page=0');
			 }
		});
		$('.tableup').on('click',function(){
			 if(true==false){
				 $('.thistable').load('/metable?page=-1');
			 }
		});
		...
		$('.thistype').on('click', function() {
			if ($(this).children().hasClass('glyphicon-triangle-bottom')) {
				$('.thistable').load('/metable?type=0&icon=glyphicon-triangle-top');
			} else {
				$('.thistable').load('/metable?type=1&icon=glyphicon-triangle-bottom');
			}
		});
		$('.thisstatus').on('click', function() {
			if ($(this).children().hasClass('glyphicon-triangle-bottom')) {
				$('.thistable').load('/metable?status=0&icon=glyphicon-triangle-top');
			} else {
				$('.thistable').load('/metable?status=1&icon=glyphicon-triangle-bottom');
			}
		});
		$('.thistime').on('click', function() {
			if ($(this).children().hasClass('glyphicon-triangle-bottom')) {
				$('.thistable').load('/metable?time=0&icon=glyphicon-triangle-top');
			} else {
				$('.thistable').load('/metable?time=1&icon=glyphicon-triangle-bottom');
			}
		});
		$('.thisvisit').on('click', function() {
			if ($(this).children().hasClass('glyphicon-triangle-bottom')) {
				$('.thistable').load('/metable?visitnum=0&icon=glyphicon-triangle-top');
			} else {
				$('.thistable').load('/metable?visitnum=1&icon=glyphicon-triangle-bottom');
			}
		});
		/* 查找 */
		$('.baseKetsubmit').on('click', function() {
			...
			console.log(baseKey);
			$('.thistable').load('/metable?baseKey='+baseKey+'');
		});
</script></div>
<script>
/* 显示表格的删除 */
