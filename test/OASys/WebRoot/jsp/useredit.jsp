...
<div class="row" style="padding-top: 15px;">
	<div class="col-md-12">
		...
		<div class="bgc-w box">
			<!--盒子头-->
			<form action="userController/update.action" method="post" onsubmit="return check();" enctype="multipart/form-data">
				 <input type="hidden" value="${user.userId}" name="userId">
				<input type="hidden" value="${user.imgPath}" name="imgPath">
					...
					<div class="box-body">
						...
						<div class="row">
							...
							<div class="col-md-6 form-group">
								...
								<input name="birthDate" class="form-control" id="start" onclick="WdatePicker()"   value="${vo.birthDate}"/>
							</div>
							<div class="col-md-6 form-group">
							...
							<div class="col-md-6 form-group">
								...
								<select class="deptselect form-control" name="dept.deptId" id="dept">
									...
									<c:forEach items="${depts}" var="dept">
										<c:if test="${dept.deptId==user.dept.deptId}">
							...
							<div class="col-md-6 form-group">
								...
								<select class="positionselect form-control" name="position.positionId" id="position">
									...
									<c:forEach var="position" items="${positions}">
										<c:if test="${user.position.positionId==position.positionId}">
							...
							<div class="col-md-6 form-group">
								<label class="control-label"> <span>皮肤</span>
									...
									<c:if test="${user.themeSkin=='blue' }">
										<option value="blue" selected="selected">天空蓝</option>
									</c:if>
									<c:if test="${user.themeSkin!='blue' }">
										<option value="blue"  >天空蓝</option>
									</c:if>
									
									<c:if test="${user.themeSkin=='green' }">
										<option value="green" selected="selected">森林绿</option>
									</c:if>
									<c:if test="${user.themeSkin!='green' }">
										<option value="green"  >森林绿</option>
									</c:if>
									
									<c:if test="${user.themeSkin=='red' }">
										<option value="red" selected="selected">大红色</option>
									</c:if>
									<c:if test="${user.themeSkin!='red' }">
										<option value="red"  >大红色</option>
									</c:if>
									
									<c:if test="${user.themeSkin=='yellow' }">
							...
							<div class="col-md-6 form-group">
								<label class="control-label"><span>用户签名</span></label> <input
									name="userSign" class="form-control" value="${user.userSign}"/>
							</div>
						</div>
	...
	$(function(){
		$("#position").click(function(){
		 ...
		 if(${user.imgPath!=null}){
		 	$("#imghead").attr("src","http://localhost:8080/upload/${user.imgPath}");
		 }
	});
</script>
  </body>
