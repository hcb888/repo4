...
import com.chaoLiuWangLuo.OASys.service.impl.evectionServiceImpl;

@Controller
@RequestMapping(value="/evectionController/")
public class evectionController {
	...
	@RequestMapping(value="processAndEvection.action")
	public String processAndEvection(Model model){
		//紧急程度
		...
		example2.createCriteria().andTypeModelEqualTo("aoa_evection");
		List<AoaTypeList> evection=typeServiceImpl.findList(example2);
		
		model.addAttribute("process", process);
		model.addAttribute("evection", evection);
		return "evection";
	}
	
	@RequestMapping(value="add.action")
	public String add(MultipartFile imgFile,AoaEvection evection,AoaProcessList process, AoaReviewed reviewed,queryVo vo) throws IllegalStateException, IOException, ParseException{
		//由于没有登陆只有死数据
				...
				//-----------------添加审核-----------------
				 ...
				 //----------------------添加出差 信息------------------
				 evection.setProcess(process);
				 //提交
				 int evectionContext=evectionServiceImpl.add(evection);
	...
	public String findById(Model model,Long id){
		...
		AoaEvection evection=null;
		if(id!=null){
			  evection=evectionServiceImpl.findById(id);
		}
